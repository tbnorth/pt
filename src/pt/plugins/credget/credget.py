...
class CredScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Input(placeholder="CredGet pass phrase", password=True, compact=True)
        for fmt in self.fmts:
            btn = Button(fmt, compact=True)
            self._btns.append(btn)
            yield btn

    def on_input_submitted(self, what):
        pass
        # decode with what.value
...
class CredGet(Button):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs["label"] = "pw1234"
        super().__init__(*args, **kwargs)
        self._screen_name = "cred_screen"

    def action_press(self) -> None:
        """Handle click events on the clock."""
        if self._screen_name not in self.app.SCREENS:
            cs = CredScreen()
            self.app.SCREENS[self._screen_name] = cs
            self.app.install_screen(cs, name=self._screen_name)

        self.app.push_screen(self._screen_name)
